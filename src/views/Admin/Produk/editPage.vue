    ...
    <v-main>
       <v-card elevation="3" class="mt-5 mx-6" style="border-radius: 6px;">
        ...
        <v-card elevation="3" style="border-radius: 6px;" class="mt-5 mx-6">
            <v-data-table :headers="headers" :items="Produk" :search="search" :items-per-page="10">
        ...
        <v-dialog transition="dialog-top-transition" max-width="600" v-model="dialog" style="border-radius: 10px;">
            <v-card style="border-radius: 10px;">
                ...
                <v-card-text class="pb-0">
                    <v-container> 
                        <v-form ref="form">
                            <v-text-field outlined color="black" class="textfield mt-3"  v-model="form.barang" label="Nama Barang" required :rules="inputRules"></v-text-field>
                            <v-text-field outlined color="black" class="textfield mt-3"  v-model="form.jenis" label="Jenis" required :rules="inputRules"></v-text-field>
                            <v-text-field outlined color="black" class="textfield"  v-model="form.harga" label="Harga" :rules="inputRules" prefix="Rp" type="numeric"></v-text-field>
                            <v-text-field outlined color="black" class="textfield"  v-model="form.ketersediaan" label="Ketersediaan" :rules="inputRules" type="numeric"></v-text-field>
                            <v-text-field outlined color="black" class="textfield"  v-model="form.id" label="Id" :rules="inputRules"></v-text-field>
                        </v-form>
                    </v-container> 
                </v-card-text>
                <v-divider></v-divider>
...
export default {
        ...
        onValue(ref(db, 'merchandises'), (snapshot) => {
            ...
            snapshot.forEach((produk) => {
                this.produk.push({
                    ...
                    artist: produk.val().jenis,
                    price: produk.val().harga,
                    stock: produk.val().ketersediaan,
                    package: produk.val().package
                });
            })
        })
    ...
    methods: {
        saveData() {
            ...
            if(this.formType == -1){
                set(ref(db, "produk/" + this.selectedId), this.form)
                .then(()=>{
                    this.snackbar = true;
        ...
        deleteData() {
            ...
            remove(ref(db, "produk/" + this.selectedId))
            .then(() => {
    ...
    computed: {
        formTitle() {
            return this.formType === 0 ? "Create Produk" : "Update Produk";
        },
    },
}
</script>